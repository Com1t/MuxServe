...
from muxserve.constants import (
    SM_HOLD_NAME_FMT,
    ADD_REQ_NAME_FMT,
    RET_REQ_NAME_FMT,
    PREEMPT_REQ_NAME_FMT,
)
from muxserve.flexserver.dist_utils import _init_distributed_environment, _setup_env
from muxserve.flexserver.llm_runtime import LLMRuntime
from muxserve.muxsched.workload_utils import Workload, Request
from muxserve.shm_utils import (
    create_shared_var,
    read_shared_var,
    write_shared_var,
    dump_to_shared_var,
    load_from_shared_var,
...
class MuxServeEngine:
    def __init__(self, llm_runtime: LLMRuntime, mps_percentage: int, is_prefill: bool):
        ...
        if self.llm_runtime.tcp_client is not None:
            sm_hold_shm_name = SM_HOLD_NAME_FMT.format(
                self.model_name, self.mps_percentage
            )
            self.sm_hold_shm = create_shared_var(sm_hold_shm_name, create=False)
    ...
    def process_output(self, outputs: List[int], batch_reqs: List[List[int]]) -> None:
        # logger.info(
        ...
        while True:
            ...
            while num_iters > 0:
                ...
                if free_request_ids:
                    ...
                    self.llm_runtime.release_request(
                        free_request_ids, finished_request_ids
                    )
                    # logger.info(
                ...
                if self.enable_profiler:
                    if self.cur_batch == self.start_batch:
                        ...
                        self.prof.stop()
                        logger.info(f"Export profiler trace to {self.prof_out_name}")
    ...
    def other_stage_loop(self):
        ...
        while True:
            ...
            if (
                self.llm_runtime.stage_id == self.llm_runtime.num_stages - 1
                and self.llm_runtime.tp_rank == 0
            ):
                if len(batch_reqs) > 0:
                    self.process_output(outputs, batch_reqs)
...
def main(args: argparse.Namespace):
    ...
    muxserve_engine = MuxServeEngine(
        llm_runtime, engine_args.mps_percentage, engine_args.is_prefill
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model-id", type=int, default=0, help="The index of served model."
    )
